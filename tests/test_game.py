...
class TestGame(unittest.TestCase):
    ...
    def test_show_header(self):
        ...
        self.assertIn("Boggle Blitz", text)
    ...
    def test_play_round_accepts_duplicate_invalid_and_done(self):
        settings = base_settings()
        with mock.patch.object(game.core, "make_grid", return_value=sample_grid()), mock.patch("builtins.input", StackedInput(["cat", "cat", "zzz", "done"])):
            result, text = self.capture(game.play_round, settings)
        self.assertEqual(result, 9)
        self.assertIn("Accepted cat", text)
        self.assertIn("Already found", text)
        self.assertIn("Invalid word", text)

    def test_play_round_hint_and_no_hint(self):
        settings = base_settings()
        with mock.patch.object(game.core, "make_grid", return_value=sample_grid()), mock.patch.object(game.core, "hint_words", side_effect=[["cat"], []]), mock.patch("builtins.input", StackedInput(["hint", "hint", "done"])):
            result, text = self.capture(game.play_round, settings)
        self.assertEqual(result, 0)
        self.assertIn("Possible words", text)
        self.assertIn("No hint", text)

    def test_play_round_quits(self):
        settings = base_settings()
        with mock.patch.object(game.core, "make_grid", return_value=sample_grid()), mock.patch("builtins.input", StackedInput(["q"])):
